...
export function ClientTopBar({ user, theme, wsToken, initialUnread }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-zinc-200/70 bg-white/80 px-6 backdrop-blur-md dark:border-zinc-800/70 dark:bg-zinc-950/70">
      <div className="hidden flex-col leading-tight md:flex">
        ...
        <span className="text-sm font-medium text-zinc-900 dark:text-zinc-50">MyTDFRIK</span>
      </div>

      <div className="ml-auto flex items-center gap-2">
        <ThemeToggle current={theme} />

        <NotificationBell portal="client" wsToken={wsToken} initialUnread={initialUnread} />
